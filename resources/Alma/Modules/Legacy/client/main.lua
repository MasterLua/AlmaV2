...
CreateThread(function()
	while not Config.Multichar do
		...
		if NetworkIsPlayerActive(PlayerId()) then
			Alma.Serv('esx:onPlayerJoined')
			Wait(1000)
			SetEntityAlpha(PlayerPedId(), 190, false)
			Wait(900)
...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	SetPedHeadBlendData(PlayerId(), 1, 1, nil, 1, 1, nil, 1, 1, nil, true)
	...
	SetEntityCoordsNoOffset(PlayerId(), ESX.PlayerData.coords.x, ESX.PlayerData.coords.y, ESX.PlayerData.coords.z + 0.25, false, false, false, true)
        NetworkResurrectLocalPlayer(ESX.PlayerData.coords.x, ESX.PlayerData.coords.y, ESX.PlayerData.coords.z, ESX.PlayerData.coords.heading, true, true, false)
	Alma.Serv('esx:onPlayerSpawn')
	TriggerEvent('esx:onPlayerSpawn')
	TriggerEvent('startSelecting') 
...
-- RegisterNetEvent('esx:ResurectPlayerOne')
-- AddEventHandler('esx:ResurectPlayerOne', function(IsMort)
-- 	local Temps = nil
-- 	if not IsMort then
-- 		Temps = 10000
-- 		return
...
-- 			while IsMort do
-- 				Wait(Temps)
-- 				ESX.ShowNotification("Appuyez sur ~b~E~s~ pour réapparaître")
...
AddEventHandler('esx:onPlayerDeath', function()
	...
	TriggerEvent("esx:ResurectPlayerOne", true)
end)
...
AddEventHandler('esx:addInventoryItem', function(item, count, showNotification)
	...
	if ESX.UI.Menu.IsOpen('default', 'Alma', 'inventory') then
		ESX.ShowInventory()
	end
end)
	...
	Citizen.CreateThread(function()
		...
		while ESX.PlayerLoaded do
			...
			if currentWeapon.timer == sleep then
				...
				Alma.Serv('esx:updateWeaponAmmo', currentWeapon.name, ammoCount)
		...
		while ESX.PlayerLoaded do
			...
			if DoesEntityExist(ESX.PlayerData.ped) then
				...
				if distance > 1 then
					...
					if not IS_IN_SELECTING then
						Alma.Serv('esx:updateCoords', formattedCoords)
					else
...
if Config.EnableDefaultInventory then
	RegisterCommand('showinv', function()
		if not ESX.PlayerData.dead and not ESX.UI.Menu.IsOpen('default', 'Alma', 'inventory') then
			ESX.ShowInventory()
		end
	end)
				...
				if distance < 1 then
					if IsControlJustReleased(0, 38) then
						if IsPedOnFoot(ESX.PlayerData.ped) and (closestDistance == -1 or closestDistance > 3) and not pickup.inRange then
							...
							Citizen.Wait(1000)
							Alma.Serv('esx:onPickup', pickupId)
							PlaySoundFrontend(-1, 'PICK_UP', 'HUD_FRONTEND_DEFAULT_SOUNDSET', false)
							Visual.Subtitle(" + "..label,1500)
						end
					end
					label = ('%s~n~%s'):format(label, '~HUD_COLOUR_G3~ E ~s~pour Rammaser')
				end
				ESX.Game.Utils.DrawText3D({x = pickup.coords.x,y = pickup.coords.y,z = pickup.coords.z + 0.15}, label, 1.1, 6)
