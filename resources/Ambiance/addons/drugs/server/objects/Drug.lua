...
setmetatable(Drug, {
    __call = function(_, harvestCoords, treatmentCoords, sellCoords, drugInfos)
        ...
        self.harvestZone = AstraSZonesManager.createPublic(harvestCoords.coords, 22, markersRGBA, function(source)
            self:harvest(source)
        end, ("Appuyez sur ~INPUT_CONTEXT~ pour récolter~n~~y~Action~s~: %ss~n~~o~Récompense~s~: %s"):format(drugInfos.harvest.interval, firstToUpper(drugInfos.harvest.reward.item).label), markersDRAW, 1.0)

        self.treatmentZone = AstraSZonesManager.createPublic(treatmentCoords.coords, 22, markersRGBA, function(source)
            self:treat(source)
        end, ("Appuyez sur ~INPUT_CONTEXT~ pour traiter~n~~y~Action~s~: %ss~n~~o~Récompense~s~: %s"):format(drugInfos.treatment.interval, firstToUpper(drugInfos.treatment.reward.item).label), markersDRAW, 1.0)
        ...
        self.sellNpc:setFloatingText(("Pssst, viens ici pour vendre: ~b~%s"):format(firstToUpper(self.drugInfos.sell.requiered.item).label), 5.5)
...
function Drug:harvest(source)
    ...
    self.setOnCoolDown(source, Astra.second(self.drugInfos.harvest.interval), function()
        ...
        TriggerClientEvent("esx:showNotification", source, ("~g~Récolte effectuée: +%i %s"):format(self.drugInfos.harvest.reward.count, firstToUpper(self.drugInfos.harvest.reward.item).label))
    end)
end
...
function Drug:treat(source)
    ...
    if actualCount < requieredCount then
        TriggerClientEvent("esx:showNotification", source, ("~r~Vous n'avez pas assez de %s (%i requis)"):format(firstToUpper(requieredItem).label, requieredCount))
        return
    end
    AstraServerUtils.toClient("playScenario", source, "CODE_HUMAN_MEDIC_TEND_TO_DEAD", Astra.second(self.drugInfos.harvest.interval), true)
    ...
    self.setOnCoolDown(source, Astra.second(self.drugInfos.treatment.interval), function()
        ...
        TriggerClientEvent("esx:showNotification", source, ("~g~Transformation effectuée: +%i %s"):format(rewardCount, firstToUpper(rewardItem).label))
    end)
end
...
function Drug:sell(source)
    ...
    self.setOnCoolDown(source, Astra.second(self.drugInfos.sell.interval), function()
        ...
        TriggerClientEvent("esx:showNotification", source, ("~g~Vente effectuée: +%i$"):format(reward))
    end)
end
