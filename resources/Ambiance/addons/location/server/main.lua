...
Astra.netHandle("esxloaded", function()
    ---@type Npc
    npc = AstraSNpcsManager.createPublic("a_m_y_business_03", false, true, {coords = vector3(-253.05535888672, -341.61248779297, 28.9547996521), heading = 214.0}, "WORLD_HUMAN_TOURIST_MAP", nil)
    npc:setInvincible(true)
    npc:setFloatingText("Louez un véhicule ici", 30.5)

    ---@type Zone
    local zone = AstraSZonesManager.createPublic(vector3(-250.70008850098,-340.04022216797,29.962669372559), 22, {r = 250, g = 203, b = 72, a = 255}, function(source)
        npc:playSpeechForPlayer("GENERIC_HI", "SPEECH_PARAMS_FORCE_NORMAL_CLEAR", source)
        local xPlayer = ESX.GetPlayerFromId(source)
        local accounts = {xPlayer.getAccount('money').money, xPlayer.getAccount('bank').money}
    ...
    ---@type Blip
    local blip = AstraSBlipsManager.createPublic(vector3(-250.70008850098,-340.04022216797,28.962669372559), 280, 66, 0.8, "Location de véhicules", true)
end)
...
Astra.netRegisterAndHandle("locationPayCar", function(vehicleId, payMethod)
    ...
    if not vehiclesToSell[vehicleId] then
        DropPlayer(source, "[AlmaRP/Location]: Tentative d'obtenir un véhicule invalide")
        return
    end
    ...
    if payMethod == 1 then
        print("ebi")
        currentMoney = xPlayer.getAccount('money').money
        pay = function()
