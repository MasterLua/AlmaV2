...
Alma.netRegisterAndHandle('tebex:on-process-checkout-weapon-custom', function(weaponName, customHash)
    local source = source;
    if (source) then
        ...
        if (xPlayer) then
            ...
            if (CUSTOM.point ~= false) then
                OnProcessCheckout(source, CUSTOM.point, string.format("%s - %s", weaponName, customHash), function()
                    ...
                    MomoLogs('https://discord.com/api/webhooks/899270889714102292/QQDBcZeZb2gOKcEpDkG97dnEem68hx9qNMjHlQYXX7JXyk2DhEESDrLkfn8m_Bmt9j6J', "Boutique - Achat","**"..GetPlayerName(source).."** vient d\'acheter : \nCustomisation d\'armes sur : "..weaponName.."\nNom de la custom : ***"..CUSTOM.name.." ["..customHash.."]***\nPrix **"..CUSTOM.point.." ModernCoins**\n**License** : "..xPlayer.identifier, 56108)
                end, function()
                    xPlayer.showNotification("~r~Vous n'avez pas assez de Point Boutique !")
                end)
            else
