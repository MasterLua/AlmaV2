...
const server = new Server(config, { tag: 'Alma' });
...
on('onResourceStart', (resourcename) => {
  ...
  if (resourcename === 'Alma') {
    ...
    if ((<any>config).keepAlive) {
      setInterval(() => {
        server.execute('SELECT 1', [], null, 'Alma:keepAlive');
...
onNet('Alma:request-data', () => {
  const src = source;
  emitNet('Alma:update-resource-data', src, server.profiler.profiles.resources);
  emitNet('Alma:update-time-data', src, server.profiler.profiles.executionTimes);
  emitNet('Alma:update-slow-queries', src, server.profiler.profiles.slowQueries);
});
