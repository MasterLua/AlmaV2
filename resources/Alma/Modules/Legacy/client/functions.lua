...
ESX.TriggerServerCallback = function(name, cb, ...)
	...
	Alma.Serv('esx:triggerServerCallback', name, ESX.CurrentRequestId, ...)
...
function ESX.IsContribWeapon(weaponName)
    ...
    if Config.ContribWeapons[weaponName] then
        return true
    end
...
ESX.Game.SpawnLocalPed = function(pedType, modelName, coords, heading, cb)
	local model = (type(modelName) == 'number' and modelName or GetHashKey(modelName))
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									if type == 'item_weapon' then
										Alma.Serv('esx:giveInventoryItem', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity and quantity > 0 and data.current.count >= quantity then
												Alma.Serv('esx:giveInventoryItem', selectedPlayerId, type, item, quantity)
												menu3.close()
												menu2.close()
												menu1.close()
											else
			...
			elseif data1.current.action == 'remove' then
				if IsPedOnFoot(playerPed) and not IsPedFalling(playerPed) then
					...
					if type == 'item_weapon' then
						...
						Citizen.Wait(1000)
						Alma.Serv('esx:removeInventoryItem', type, item)
					else
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
							...
							if quantity and quantity > 0 and data.current.count >= quantity then
								...
								TaskPlayAnim(playerPed, dict, anim, 8.0, 1.0, 1000, 16, 0.0, false, false, false)
								Citizen.Wait(1000)
								Alma.Serv('esx:removeInventoryItem', type, item, quantity)
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
						end, function(data2, menu2)
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										Alma.Serv('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
