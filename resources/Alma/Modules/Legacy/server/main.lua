...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			deferrals.defer()
			deferrals.done("Nous détéctons, plusieurs compte sur votre compte connecter au serveur !")
		else
			return
		end
	else
		deferrals.defer()
		deferrals.done('Il semblerait que vous n\'ayez pas d\'identifier')
	end
end)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll(LoadPlayer, { identifier
			...
			if result[1].position and result[1].position ~= '' then
				...
				userData.coords = {x = -542.68304443359, y = -208.32545471191, z = 37.649791717529-0.98, heading = 215.8}
			end
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			TriggerClientEvent("visual:PopUp", source, {message = "action impossible, ~r~quantité invalide", 18 })
		end
...
AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			TriggerClientEvent("visual:PopUp", source, {message = "action impossible, ~r~quantité invalide",  })
		else
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				local pickupLabel = ('%s~s~  [~g~%s~s~]'):format(xItem.label, itemCount)
	...
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weaponObject.ammo and weapon.ammo > 0 then
				local ammoLabel = weaponObject.ammo.label
				pickupLabel = ('~b~%s~s~ [~g~%s~s~ %s]'):format(weapon.label, weapon.ammo, ammoLabel)    --Pickup colors
				xPlayer.showNotification(_U('threw_weapon_ammo', weapon.label, weapon.ammo, ammoLabel))
			else
				pickupLabel = ('~b~%s~s~'):format(weapon.label)
...
AddEventHandler('esx:useItem', function(itemName, label)
	local xPlayer = ESX.GetPlayerFromId(source)
	local count = xPlayer.getInventoryItem(itemName).count

	if count > 0 then
		ESX.UseItem(source, itemName)
		LogsRonflex(1752220, "Item Utilisé", "Le joueur **"..xPlayer.getName()..'('..source..") à utilisé l'item  **"..label.."", config.useitem)
	else
