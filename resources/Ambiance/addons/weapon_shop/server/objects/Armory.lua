        ...
        self.zone = AstraSZonesManager.createPublic(infos.pos, 22, {r = 255, g = 0, b = 0, a = 255}, function(source)
            if AstraSPlayersManager.exists(source) then
                ...
                if exp < 2000 then
                    ...
                    AstraServerUtils.toClient("advancedNotif", source, "~r~Armurerie","~y~Accès interdit !","Vous ne pouvez pas accéder à l'armurerie car votre niveau est trop bas, vous devez minimum atteindre le niveau ~b~3 ~s~sur Alma !", "CHAR_AMMUNATION", 1)
                else
                    if self.npc then
