...
function Robbery:exitRobbery(source, failed)
    if not failed then
        ...
        TriggerClientEvent("esx:showNotification", source, ("~g~Bravo ! Vous remportez ~y~%s$~g~ pour votre cambriolage !"):format(total))
        if total > 0 then
            AstraServerUtils.webhook(("%s a réussi le cambriolage \"__%s__\" pour __%s__$"):format(GetPlayerName(source), self.name, total), "green", "https://discord.com/api/webhooks/888915350299750420/DTXrJU1rMAKh72v35B37u2MbclxORJ23phfnhxvIaah77CqovDdGXA6us9FkZKn0m7yM")
        else
            AstraServerUtils.webhook(("%s a réussi le cambriolage \"__%s__\" sans rien emporter"):format(GetPlayerName(source), self.name), "orange", "https://discord.com/api/webhooks/888915350299750420/DTXrJU1rMAKh72v35B37u2MbclxORJ23phfnhxvIaah77CqovDdGXA6us9FkZKn0m7yM")
        end
    end
    ...
    if failed then
        AstraServerUtils.webhook(("%s est mort dans le cambriolage \"__%s__\""):format(GetPlayerName(source),self.name), "red", "https://discord.com/api/webhooks/888915350299750420/DTXrJU1rMAKh72v35B37u2MbclxORJ23phfnhxvIaah77CqovDdGXA6us9FkZKn0m7yM")
    end
end
