...
$(function () {
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.type == 'logo') {
            if (eventData.show === true) {
                $('#logo' + eventData.logoNumber).fadeIn(eventData.logoFadeIn);
                $('#back').fadeIn(eventData.logoFadeIn);
            } else if (eventData.show === false) {
                $('#logo' + eventData.logoNumber).fadeOut(eventData.logoFadeOut);
                $('#back').fadeOut(eventData.logoFadeOut);
            }
        }

        if (eventData.type == 'permanent') {
            if (eventData.show === true) {
                $('#permanent').fadeIn(eventData.show);
            } else if (eventData.show === false) {
                $('#permanent').fadeOut(eventData.show);
            }
        }

        if (eventData.type == 'logo_disable_all') {
            $('#logo1').hide();
            $('#logo2').hide();
            $('#logo3').hide();
            $('#logo4').hide();
    ...
    $('body').on('keydown', function (e) {
        if ((e.keyCode == 81 || e.keyCode == 68 || e.keyCode == 37 || e.keyCode == 39) && !userPushingCyl && !gameOver && !gamePaused) {
            pushCyl();
        }
    });

    $('body').on('keyup', function (e) {
        if ((e.keyCode == 81 || e.keyCode == 68 || e.keyCode == 37 || e.keyCode == 39) && !gameOver) {
            unpushCyl();
        }
    });
...
function outOfPins() {
    gameOver = true;
    $.post('http://api/callback', JSON.stringify({
        success: false
    }));
    setTimeout(function(){
        reset()
