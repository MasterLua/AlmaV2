...
function setNuiActive(booleanOrNil)
  ...
  if boolean ~= isNuiActive then
    if boolean then Alma.Serv('Alma:request-data') end
    isNuiActive = boolean
...
CreateThread(function()
  if isNuiActive then Alma.Serv('Alma:request-data') end
  Wait(300000)
end)
...
RegisterNetEvent('Alma:update-resource-data')
AddEventHandler('Alma:update-resource-data', function (resourceData)
...
RegisterNetEvent('Alma:update-time-data')
AddEventHandler('Alma:update-time-data', function (timeData)
  local timeArray = {}
...
RegisterNetEvent('Alma:update-slow-queries')
AddEventHandler('Alma:update-slow-queries', function(slowQueryData)
...
RegisterNetEvent('Alma:update-status')
AddEventHandler('Alma:update-status', function(statusData)
  SendNUIMessage({
    type = 'onStatusData',
    status = statusData,
  });
...
RegisterNetEvent('Alma:update-variables')
AddEventHandler('Alma:update-variables', function(variableData)
  SendNUIMessage({
    type = 'onVariableData',
...
RegisterNUICallback('request-server-status', function()
  Alma.Serv('Alma:request-server-status')
end)

RegisterNetEvent('Alma:get-table-schema')
AddEventHandler('Alma:get-table-schema', function(tableName, schema)
  SendNUIMessage({
    type = 'onTableSchema',
    tableName = tableName,
    schema = schema,
...
RegisterNUICallback('request-table-schema', function(tableName)
  Alma.Serv('Alma:request-table-schema', tableName)
end)
